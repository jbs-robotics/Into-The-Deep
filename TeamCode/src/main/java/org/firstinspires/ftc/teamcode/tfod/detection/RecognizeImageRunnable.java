...
package org.firstinspires.ftc.teamcode.tfod.detection;
 ...
 * image, directly out of a {@link com.google.ftcresearch.tfod.generators.FrameGenerator} into a list of
 * {@link Recognition}.
 */
class RecognizeImageRunnable implements Runnable {
  ...
  private static ByteBuffer preprocessFrame(YuvRgbFrame frame, int imageSize, boolean
    ...
    timer.start("Overall Preprocessing");

    timer.start("Resizing image");
    YuvRgbFrame smallFrame = frame.resize(new Size(imageSize, imageSize));
    int[] rgbArray = smallFrame.getRgbData().array();
    timer.end();
    ...
    timer.start("Copy data into bytebuffer pixel by pixel");
  ...
  private void postprocessDetections() {
    ...
    Log.i(TAG, "Received " + numDetections[0] + " detections!");
    ...
    for (int i = 0; i < params.maxNumDetections; i++) {
      ...
      if (detectedClass >= labels.size() || detectedClass < 0) {
        Log.w(TAG, "Got a recognition with an invalid / background label");
        continue;
      }
      ...
      annotatedFrame.getRecognitions().add(new Recognition(
          labels.get(detectedClass), detectionScore, detectionBox
      ));
    }

    // The recognitions need to be in sorted order, decreasing by confidence.
    Collections.sort(annotatedFrame.getRecognitions(), (a, b) -> Float.compare(b.getConfidence(),
        a.getConfidence()));
  }

  @Override
  public void run() {
    Log.d(TAG, "Preprocessing bitmap");
    ...
    Log.d(TAG, "Creating input / output arrays");
    ...
    Log.d(TAG, "Starting to run the interpreter");
